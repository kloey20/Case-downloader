#!/usr/bin/env python3
"""
LawPhil Case Downloader - Multi-Browser Version
...
class LawPhilDownloader:
    ...
    def setup_driver(self):
        ...
        # Shared arguments for any Chromium-based browser
        common_args = [
            '--no-sandbox',
        ...
        try:
            ...
            # If Chrome isn't installed or fails, silently log and move to Edge
            pass
        ...
        try:
            ...
            raise RuntimeError(f"Could not find a compatible Chromium browser (Chrome or Edge): {e}")
    ...
    def save_page_as_pdf(self, url, output_filename=None):
        ...
        try:
            ...
            # This executes flawlessly on both Chrome and Edge
    ...
    def download_case(self, search_input, output_filename=None):
        try:
            ...
            if not case_url:
                print("✗ No matching case found on LawPhil.")
                return None
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Download LawPhil cases as PDF')
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('search_term', nargs='?', help='Case number')
    group.add_argument('--url', help='Direct URL')
    ...
    if args.url:
        path = downloader.download_from_url(args.url, args.output)
    else:
        path = downloader.download_case(args.search_term, args.output)
